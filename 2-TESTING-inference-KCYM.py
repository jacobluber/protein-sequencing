...
for arg_list in input_args:
    ...
    # print(df.groupby('seq_label')['Matching Percentage'].agg(['mean', 'count', 'std']))
    
    # delete model & pipeline else db can't read without GPU
    # model = None
    # pipe = None
    # dill.dump_session(f'{outname}-F{c}.db')
    # df.to_csv(f'{outname}-F{c}.csv')
